...
#define FASTLIO_PSINS2_H
#ifdef FASTLIO_PSINS2_H
...
bool psins2::INS_Data_Ready()
{
    ...
    {
        wib[i] = wib[i] - wib_bia;
        fib[i] = fib[i] * Grav / acc_mean.norm();
        fib[i] = fib[i] - fib_bia;
    }
...
bool psins2::INS_Att_Predict()
{
    ...
    double cos_alpha = cos(mag_alpha);
...
bool psins2::INS_RotScull()
{
    ...
    Eigen::Vector3d  dw, da, daw;
    Eigen::Vector3d w1, w2, f1, f2;
    w1 = wib[1]; w2 = wib[0]; 
    dw = (w2 - w1) / dt;
    ...
    // ROS_INFO("dw: %7.4f  %7.4f  %7.4f \n",dw.x(),dw.y(),dw.z());
    // ROS_INFO("da: %7.4f  %7.4f  %7.4f \n",da.x(),da.y(),da.z());
    // ROS_INFO("daw: %7.4f  %7.4f  %7.4f \n",daw.x(),daw.y(),daw.z());
