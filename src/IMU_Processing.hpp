    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  cov_acc       = V3D(0.1, 0.1, 0.1);   //加速度测量协方差初
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  for (const auto &imu : meas.imu)    //遍历测量信息中存放的imu信息
  {
    ...
    /*简单介绍一下这个求平均//正推倒推均可
      ...
      N = 2时 mean2 = (cur2 + cur1)/2 = (cur2 + mean1)/2 =  mean1 + (cur2 - mean1)/2 //这里实现了将代码简约化类推
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  imu_state = kf_state.get_x();           //更新IMU状态，以便于下一帧
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    acc_imu<<VEC_FROM_ARRAY(tail->acc);       //拿到后一帧的IMU加速
